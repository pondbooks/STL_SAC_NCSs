...
class STL_Problem(gym.Env):
    ...
    def __init__(self):
        ...
        self.tau = int(hrz_phi + 1) # hrz(phi) + 1 = 140
        ...
        self.low_extended_state_space = -np.ones(self.tau*self.car_dim + self.num_of_past_actions*2) # 440
        self.high_extended_state_space = np.ones(self.tau*self.car_dim + self.num_of_past_actions*2) # 440
        ...
        # varphi_{1}, varphi_{2}, and varphi_{3} ##################
        self.init_low_x = 1.5
        self.init_low_y = 1.5
        self.init_high_x = 3.5
        self.init_high_y = 3.5

        self.stl_1_low_x = 0.5
        self.stl_1_low_y = 0.5
        self.stl_1_high_x = 1.5
        self.stl_1_high_y = 1.5

        self.stl_2_low_x = 3.5
        self.stl_2_low_y = 3.5
        self.stl_2_high_x = 4.5
        self.stl_2_high_y = 4.5

        self.stl_3_low_x = 0.0
        self.stl_3_low_y = 0.0
        self.stl_3_high_x = 5.0
        self.stl_3_high_y = 5.0
        # ############################################################
    ...
    def reset(self): 
        ...
        # past trajectory memory [s_{t-tau+1},s_{t-tau+2},...,s_{t}]
        self.past_state_trajectory = [] 

        # past action memory [a_{t-delta},a_{t-delta+1},...,a_{t-1}]
        self.past_action_list = [] 

        for i in range(self.tau): # The initial past state trajectory is [s_0,s_0,...,s_0]
    ...
    def step(self, action): 
        ...
        true_action = self.past_action_list[len(self.past_action_list)-self.network_delay]
        ...
        #print('next_state:'+str(self.state))
        #print('tau_state:'+str(self.past_state_trajectory))

        self.past_state_trajectory = self.past_state_trajectory[1:]
        self.past_state_trajectory.append(self.state.copy())
        #print('next_past_states:'+str(self.past_state_trajectory))
    ...
    def observe(self, tau_state, delta_actions): 
        tau_num = len(tau_state)
        delta_num = len(delta_actions)
        assert tau_num == self.tau, "dim of tau-state is wrong."
        assert delta_num == self.num_of_past_actions, "dim of delta-action is wrong." 

        # Define tau-delta extended state
        obs = np.zeros(tau_num*self.car_dim + delta_num*2) # 140*3 + 10*2 = 540
        ...
        for j in range(delta_num):
            obs[self.car_dim*tau_num + 2*j] = delta_actions[j][0]
            obs[self.car_dim*tau_num + 2*j + 1] = delta_actions[j][1]
        return obs
    ...
    def reward(self, tau_state): 
        ...
        phi_3_rob = self.subSTL_3_robustness(tau_state[tau_num-1]) # phi_{3}

        returns = min(phi_1_rob, phi_2_rob) # F phi_{1} \land F phi_{2}
        returns = min(returns, phi_3_rob) # F phi_{1} \land F phi_{2} \land phi_{3}
    ...
    def evaluate_stl_formula(self): # For evaluation (Success Checking)
        if self.num_steps >= self.tau-1: 
            ...
            phi_3_rob = self.subSTL_3_robustness(self.past_state_trajectory[tau_num-1])

            returns = min(phi_1_rob, phi_2_rob)
            returns = min(returns, phi_3_rob)
    ...
    def subSTL_3_robustness(self,state):
        psi1 = state[0] - self.stl_3_low_x  
        psi2 = self.stl_3_high_x - state[0]
        psi3 = state[1] - self.stl_3_low_y 
        psi4 = self.stl_3_high_y - state[1]
        robustness = min(psi1, psi2)
        robustness = min(robustness, psi3)
        robustness = min(robustness, psi4)
        return robustness
...
class STL_Problem_Preprocess(gym.Env):
    ...
    def step(self, action): 
        ...
        #print('state:'+str(self.state))
        #print('action:'+str(action))
        #print('delta actions'+str(self.past_action_list))
        ...
        self.past_state_trajectory.append(self.state.copy())
        #print('next_extended_state:'+str(self.past_state_trajectory))
    ...
    def observe(self, tau_state, delta_actions): # return wrapped tau_mdp state
        tau_num = len(tau_state)
        delta_num = len(delta_actions)
        assert tau_num == self.tau, "dim of tau-state is wrong."
        assert delta_num == self.num_of_past_actions, "dim of delta-action is wrong." 
        ...
        for j in range(delta_num):
            obs[5 + 2*j] = delta_actions[j][0]
            obs[6 + 2*j] = delta_actions[j][1]
        return obs
